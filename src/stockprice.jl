"""
    company_quote(fmp, symbol)
    company_quote(fmp, symbols)

Returns the company quote for the specified symbol(s). Each element is a dictionary.
...
# get the company quote for AAPL
data = company_quote(fmp, "AAPL")

# get the company quote for MSFT and GOOG
data = company_quote(fmp, ["MSFT", "GOOG"])
```
"""
function company_quote(fmp::FMP, symbol::String)::Vector{Any}
...
function company_quote(fmp::FMP, symbols::Vector{String})::Vector{Any}
...
Returns the otc quote for the specified symbol(s). Each element is a dictionary.
...
data = price_change(fmp, ["BATRB", "FWONB"])
```
"""
...
Returns the price quote for the specified symbol. Each element is a dictionary.

# Arguments
- fmp::FMP: A Financial Modeling Prep instance.
- symbol::String: A stock symbol.

See [Price-Quote](https://site.financialmodelingprep.com/developer/docs/#Stock-Price) for more details.
...
"""
    price_quotes(fmp, exchange)

Returns the price quotes for the specified exchange. Each element is a dictionary.

# Arguments
- fmp::FMP: A Financial Modeling Prep instance.
- exchange::String: A stock exchange.

See [Price-Quotes](https://site.financialmodelingprep.com/developer/docs/#Stock-Price) for more details.
...
```
"""
function price_quotes(fmp::FMP, exchange::String)::Vector{Any}
    endpoint = "quote-short/$(exchange)"
    url, query = Client.make_url_v3(fmp, endpoint)
    response = Client.make_get_request(url, query)
    data = Client.parse_json_response(response)
    return data
...
See [Historical-Price-Quote](https://site.financialmodelingprep.com/developer/docs/#Stock-Historical-Price) for more details.
...
data = historical_price_quote(fmp, "AAPL", TIME_FREQUENCIES.minutes15)
